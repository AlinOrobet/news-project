...
const NewsDetails = () => {
  ...
  const {id, title, description, image, author, content, date} = adaptedNewsDetails;

  const formattedDate = formatDate(date);

  const isFavorite = (id) => {
    const newsFound = favoritesState.news.find((newsItem) => newsItem.id === id);
  ...
  const handleFavoritesClick = () => {
    ...
    const actionResult = newsFound ? removeFromFavorites(id) : addToFavorites(newsObj);
    ...
    <Layout>
        ...
        <Row className="d-flex justify-content-center">
          <Col xs={12} lg={8}>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
              ...
              <Button
                variant={isFavorite(id) ? "btn btn-outline-danger" : "btn btn-primary"}
                onClick={handleFavoritesClick}
              >
                {isFavorite(id) ? "Sterge din Favorite" : "Adauga in Favorite"}
              </Button>
            </div>
      ...
      {alertVision ? (
        <div className="alertContainer">
          <Alert id="alertNotification" variant={isFavorite(id) ? "success" : "danger"}>
            {isFavorite(id)
